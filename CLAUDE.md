...
- 6-way facing verified end to end (2026-09-15): `minecraft:facing_direction` stores the player's look direction including `up`/`down` (front = opposite of state, same semantics as `cardinal_direction`); rotation signs in placer.json are correct as written (state `up` → `[90,0,0]`, `down` → `[-90,0,0]`); vertical fronts place above/below and the hopper front-exclusion holds. **Deliberate aesthetic decision:** the whole-block rotation look (side textures rotate with the block) is PREFERRED by the maintainer over vanilla dispenser/dropper's dedicated vertical-facing textures — do not add per-permutation material instances to imitate vanilla. Placers placed before the state switch need breaking and re-placing.
