...
Pending runtime verification (Phase 4 hoppers, implemented 2026-09-08 — move up once confirmed):

- The `minecraft:tick` component fires `onTick` every 8 ticks and permits world mutation (container writes, dynamic-property writes) inside the handler.
- Hopper `facing_direction`/`toggle_bit` states read correctly via `permutation.getState`, and hopper containers accept/yield items via `BlockInventoryComponent`.
- Front-face insertion is rejected; above/side insertion and below extraction work at the vanilla rate.
