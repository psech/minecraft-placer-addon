...
- `onPlayerBreak` allows world mutation synchronously (verified 2026-08-30): `dimension.spawnItem` and dynamic-property writes work directly inside the hook — no `system.run` deferral needed. Contents drop on player break like a vanilla container. Remaining Phase 6 gap: destruction that bypasses the hook (explosions, destroy-mode commands) orphans the stored inventory — the `onBreak` hook (fires for all destruction causes, v26.40 docs) is the candidate fix.
...
- Forms cannot be updated in place, so live UI updates during hopper transfers are simulated: `openScreens` in `main.js` tracks who is viewing which Placer; when a transfer changes a watched Placer, `uiManager.closeAllForms` (stable in `@minecraft/server-ui` 2.1.0) force-closes the form with a `refresh` flag set, and the response handler reopens it with fresh contents — the zero-motion screen animations make this read as an in-place refresh.

Pending runtime verification (UI live refresh, implemented 2026-09-08 — move up once confirmed):

- `uiManager.closeAllForms` works on 1.26.45 and the forced-close resolves the form promise (canceled), triggering the flagged reopen.
- The refresh looks in-place (no visible flicker beyond a frame); hover/cursor focus resets are acceptable.
